...
bool CoreChecks::ValidateGraphicsPipeline(const vvl::Pipeline &pipeline, const void *pipeline_ci_pnext,
    ...
    if (pipeline.descriptor_heap_embedded_samplers_count > 0) {
        skip |= ValidateEmbeddedSamplersCount("VUID-vkCreateGraphicsPipelines-pCreateInfos-11414", create_info_loc);
        skip |= ValidateEmbeddedSamplersCount("VUID-vkCreateGraphicsPipelines-pCreateInfos-11429",
    ...
    for (uint32_t i = 0; i < library_create_info.libraryCount; ++i) {
        ...
        if (has_desc_heap_flags != lib_has_desc_heap_flags) {
            if (has_desc_heap_flags) {
                ...
                skip |=
                    LogError("VUID-VkGraphicsPipelineCreateInfo-flags-11274", device, library_loc,
