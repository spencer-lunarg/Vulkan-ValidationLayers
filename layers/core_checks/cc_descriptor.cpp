...
bool CoreChecks::PreCallValidateCreateSampler(VkDevice device, const VkSamplerCreateInfo *pCreateInfo,
    ...
    if (device_state->descriptor_heap_global_embedded_sampler_count_ > 0) {
        skip |= ValidateEmbeddedSamplersCount("VUID-vkCreateSampler-maxSamplerAllocationCount-11412", error_obj.location);
    }
...
bool CoreChecks::PreCallValidateWriteSamplerDescriptorsEXT(VkDevice device, uint32_t samplerCount,
    ...
    if (!enabled_features.descriptorHeap) {
        skip |= LogError("VUID-vkWriteSamplerDescriptorsEXT-descriptorHeap-11202", device, error_obj.location,
                         "descriptorHeap feature was not enabled.");
    }

    for (uint32_t i = 0; i < samplerCount; ++i) {
        const VkHostAddressRangeEXT& descriptor_range = pDescriptors[i];
        if (descriptor_range.size < phys_dev_ext_props.descriptor_heap_props.samplerDescriptorSize) {
            skip |= LogError("VUID-vkWriteSamplerDescriptorsEXT-size-11203", device,
                             ...
                             "(%" PRIu64 ") is less than the size of a samplerDescriptorSize (%" PRIu64 ").", descriptor_range.size,
                             phys_dev_ext_props.descriptor_heap_props.samplerDescriptorSize);
        }
        if (vku::FindStructInPNextChain<VkSamplerYcbcrConversionInfo>(pSamplers[i].pNext)) {
            skip |= LogError("VUID-vkWriteSamplerDescriptorsEXT-pSamplers-11204", device,
                             error_obj.location.dot(Field::pSamplers, i).dot(Field::pNext),
                             "cannot contain a VkSamplerYcbcrConversionInfo structure.");
        }
        if (pSamplers[i].borderColor == VK_BORDER_COLOR_FLOAT_CUSTOM_EXT ||
            pSamplers[i].borderColor == VK_BORDER_COLOR_INT_CUSTOM_EXT) {
            const auto bc = vku::FindStructInPNextChain<VkSamplerCustomBorderColorIndexCreateInfoEXT>(pSamplers[i].pNext);
            if (!bc) {
                skip |= LogError("VUID-vkWriteSamplerDescriptorsEXT-borderColor-11444", device,
                                 error_obj.location.dot(Field::pSamplers, i).dot(Field::borderColor),
                                 "is %s, but VkSamplerCustomBorderColorIndexCreateInfoEXT is missing.",
                                 string_VkBorderColor(pSamplers[i].borderColor));
                ...
                skip |= LogError("VUID-vkWriteSamplerDescriptorsEXT-borderColor-11205", device,
                                 error_obj.location.dot(Field::pSamplers, i)
                                     .pNext(Struct::VkSamplerCustomBorderColorIndexCreateInfoEXT, Field::index),
                                 "(%" PRIu32 ") is greater than or equal to maxCustomBorderColorSamplers (%" PRIu32 ").", bc->index,
                                 phys_dev_ext_props.custom_border_color_props.maxCustomBorderColorSamplers);
            }
        }
        const auto* object_name = vku::FindStructInPNextChain<VkDebugUtilsObjectNameInfoEXT>(pSamplers[i].pNext);
        if (object_name && object_name->objectType != VK_OBJECT_TYPE_UNKNOWN) {
            skip |= LogError("VUID-vkWriteSamplerDescriptorsEXT-pNext-11400", device,
                             error_obj.location.dot(Field::pSamplers, i).dot(Field::pNext),
                             "contains VkDebugUtilsObjectNameInfoEXT structure with objectType %s.",
                             string_VkObjectType(object_name->objectType));
        }
        skip |= ValidateSamplerCreateInfo(pSamplers[i], error_obj.location.dot(Field::pSamplers, i));
    }
...
bool CoreChecks::PreCallValidateWriteResourceDescriptorsEXT(VkDevice device, uint32_t resourceCount,
    ...
    for (uint32_t i = 0; i < resourceCount; ++i) {
        ...
        const Location resource_data_loc = resource_loc.dot(Field::data);
        const VkDescriptorType desc_type = pResources[i].type;
        const bool is_texel_buffer = IsDescriptorHeapTexelBuffer(desc_type);
        const bool is_image = IsDescriptorHeapImage(desc_type);
        // TODO - Cache these once on device creation
        // The main thing to be careful of is |bufferDescriptorSize| is not for texel buffer
        const VkDeviceSize expected_size = DispatchGetPhysicalDeviceDescriptorSizeEXT(physical_device, desc_type);

        const VkHostAddressRangeEXT& descriptor_range = pDescriptors[i];
        if (descriptor_range.size < expected_size) {
            skip |= LogError("VUID-vkWriteResourceDescriptorsEXT-size-11207", device,
                             error_obj.location.dot(Field::pDescriptors, i).dot(Field::size),
                             "(%" PRIu64 ") is less than size expected for %s (%" PRIu64
                             ") when queried to vkGetPhysicalDeviceDescriptorSizeEXT.",
                             descriptor_range.size, string_VkDescriptorType(desc_type), expected_size);
        }

        const VkImageDescriptorInfoEXT* image_info = pResources[i].data.pImage;
        if (is_image && image_info && image_info->pView) {
            ...
            if (image_state->create_info.imageType == VK_IMAGE_TYPE_3D) {
                if (IsValueIn(desc_type, {VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT, VK_DESCRIPTOR_TYPE_SAMPLE_WEIGHT_IMAGE_QCOM,
                                          VK_DESCRIPTOR_TYPE_BLOCK_MATCH_IMAGE_QCOM})) {
                    skip |= LogError("VUID-VkResourceDescriptorInfoEXT-type-11422", image_view_ci.image,
                                     resource_data_loc.dot(Field::pImage).dot(Field::pView).dot(Field::image),
                                     "has been created with VK_IMAGE_TYPE_3D\npResources[%" PRIu32 "].type = %s", i,
                                     string_VkDescriptorType(desc_type));
                }
                if (image_view_ci.viewType == VK_IMAGE_VIEW_TYPE_2D &&
                    ...
                    skip |= LogError("VUID-VkImageDescriptorInfoEXT-pView-11427", image_view_ci.image,
                                     resource_data_loc.dot(Field::pImage).dot(Field::pView).dot(Field::viewType),
                                     ...
                                     i, string_VkDescriptorType(desc_type));
                }
                ...
                if (desc_type == VK_DESCRIPTOR_TYPE_STORAGE_IMAGE && image_view_ci.viewType == VK_IMAGE_VIEW_TYPE_2D &&
                ...
                if (desc_type == VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE && image_view_ci.viewType == VK_IMAGE_VIEW_TYPE_2D &&
                    !enabled_features.sampler2DViewOf3D) {
                ...
                if (aspect_mask & VK_IMAGE_ASPECT_DEPTH_BIT) {
                    if (aspect_mask & VK_IMAGE_ASPECT_STENCIL_BIT) {
                        skip |= LogError("VUID-VkImageDescriptorInfoEXT-pView-11430", image_view_ci.image,
                                         resource_data_loc.dot(Field::pImage).dot(Field::pView).dot(Field::image),
                                         "is a depth/stencil image (%s), but aspectMask (%s) has both STENCIL and DEPTH aspect set",
                                         string_VkFormat(image_state->create_info.format),
                                         string_VkImageAspectFlags(image_view_ci.subresourceRange.aspectMask).c_str());
                    }
                } else if ((aspect_mask & VK_IMAGE_ASPECT_STENCIL_BIT) == 0) {
                    skip |= LogError("VUID-VkImageDescriptorInfoEXT-pView-11430", image_view_ci.image,
                                     resource_data_loc.dot(Field::pImage).dot(Field::pView).dot(Field::image),
                                     "is a depth/stencil image (%s), but aspectMask (%s) is missing a STENCIL or DEPTH aspects",
                                     string_VkFormat(image_state->create_info.format),
                                     string_VkImageAspectFlags(image_view_ci.subresourceRange.aspectMask).c_str());
                }
            }

            skip |= ValidateImageViewCreateInfo(image_view_ci, resource_data_loc.dot(Field::pImage).dot(Field::pView));
            if (IsValueIn(desc_type, {VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,
                                      VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT})) {
                VkImageUsageFlags usage = desc_type == VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE   ? VK_IMAGE_USAGE_SAMPLED_BIT
                                          : desc_type == VK_DESCRIPTOR_TYPE_STORAGE_IMAGE ? VK_IMAGE_USAGE_STORAGE_BIT
                                                                                          : VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT;
                if ((image_state->create_info.usage & usage) == 0) {
                    const char* vuid = desc_type == VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE ? "VUID-VkResourceDescriptorInfoEXT-type-11458"
                    ...
                    skip |= LogError(
                        vuid, image_state->VkHandle(), resource_data_loc.dot(Field::pImage).dot(Field::pView).dot(Field::image),
                        "was created with usages %s, but required %s is missing.",
                        string_VkImageUsageFlags(image_state->create_info.usage).c_str(), string_VkImageUsageFlags(usage).c_str());
                }
            }
        }

        if (is_texel_buffer && pResources[i].data.pTexelBuffer) {
            const VkDeviceAddressRangeEXT& address_range = pResources[i].data.pTexelBuffer->addressRange;
            const VkBufferUsageFlags2 buffer_usage = (desc_type == VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER)
                                                         ? VK_BUFFER_USAGE_2_UNIFORM_TEXEL_BUFFER_BIT
                                                         : VK_BUFFER_USAGE_2_STORAGE_TEXEL_BUFFER_BIT;
            const char* usage_vuid = (desc_type == VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER)
            ...
            skip |= ValidateDeviceAddressRange(address_range.address, address_range.size, false,
                                               resource_data_loc.dot(Field::pTexelBuffer).dot(Field::addressRange),
                                               LogObjectList(device), buffer_usage, usage_vuid);
        } else if (desc_type == VK_DESCRIPTOR_TYPE_SAMPLE_WEIGHT_IMAGE_QCOM ||
                   desc_type == VK_DESCRIPTOR_TYPE_BLOCK_MATCH_IMAGE_QCOM || desc_type == VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT) {
            if (!image_info) {
                skip |= LogError("VUID-VkResourceDescriptorInfoEXT-type-11469", device, resource_data_loc.dot(Field::pImage),
                                 "must not be NULL for descriptor type %s.", string_VkDescriptorType(desc_type));
            }
        }

        if (IsDescriptorHeapAddr(desc_type) && pResources[i].data.pAddressRange) {
            const VkDeviceAddressRangeEXT& address_range = *pResources[i].data.pAddressRange;
            if (desc_type == VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR) {
                bool found_covering_as = false;
                VkAccelerationStructureKHR out_of_bounds_as = VK_NULL_HANDLE;
                VkBuffer out_of_bounds_buffer = VK_NULL_HANDLE;
                VkDeviceAddress buffer_start = 0;
                VkDeviceAddress buffer_size = 0;
                VkAccelerationStructureKHR uncovered_as = VK_NULL_HANDLE;
                VkBuffer uncovered_buffer = VK_NULL_HANDLE;
                bool memory_bound = true;
                bool full_bound = true;
                device_state->AnyOf<vvl::AccelerationStructureKHR>([&](const vvl::AccelerationStructureKHR& s) {
                    if (s.buffer_state.get()) {
                        const VkDeviceAddress range_begin = address_range.address;
                        const VkDeviceSize range_end = range_begin + address_range.size;
                        const VkDeviceAddress buffer_begin = s.buffer_device_address;
                        const VkDeviceSize buffer_end = buffer_begin + s.buffer_state->create_info.size;
                        if (buffer_begin <= range_begin && range_begin <= buffer_end) {
                            // We found AS that contains start of range
                            if (range_end <= buffer_end) {
                                found_covering_as = true;
                            } else {
                                out_of_bounds_buffer = s.buffer_state->VkHandle();
                                out_of_bounds_as = s.VkHandle();
                                buffer_start = buffer_begin;
                                buffer_size = s.buffer_state->create_info.size;
                            }
                            if (address_range.size > 0 && !s.buffer_state->sparse &&
                                (!s.buffer_state->IsMemoryBound() || !s.buffer_state->HasFullRangeBound())) {
                                uncovered_as = s.VkHandle();
                                uncovered_buffer = s.buffer_state->VkHandle();
                                memory_bound = s.buffer_state->IsMemoryBound();
                                full_bound = s.buffer_state->HasFullRangeBound();
                            }
                        }
                    }

                    return found_covering_as;
                });
                if (!found_covering_as && out_of_bounds_as != VK_NULL_HANDLE) {
                    skip |= LogError(
                        "VUID-VkDeviceAddressRangeEXT-address-11365", LogObjectList(out_of_bounds_as, out_of_bounds_buffer),
                        resource_data_loc.dot(Field::pAddressRange).dot(Field::size),
                        "is %" PRIu64 ", that is steps out buffer boundary size %" PRIu64 ".\nrange start address 0x%" PRIx64
                        "\nacceleration structure buffer start address 0x%" PRIx64 ".",
                        address_range.size, buffer_size, address_range.address, buffer_start);
                }

                BufferAddressValidation<1> buffer_address_validator = {
                    {{{"VUID-VkResourceDescriptorInfoEXT-type-11465",
                       [](const vvl::Buffer& buffer_state) {
                ...
                skip |= buffer_address_validator.ValidateDeviceAddress(
                    *this, resource_data_loc.dot(Field::pAddressRange).dot(Field::address), LogObjectList(device),
                    address_range.address);

            } else if (desc_type == VK_DESCRIPTOR_TYPE_STORAGE_BUFFER || desc_type == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER) {
                if (address_range.size == 0) {
                    skip |= LogError("VUID-VkResourceDescriptorInfoEXT-type-11433", device,
                                     resource_data_loc.dot(Field::pAddressRange).dot(Field::size),
                                     "is zero. (address is 0x%" PRIx64 ")", address_range.address);
                }

                const VkBufferUsageFlags2 buffer_usage = (desc_type == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER)
                                                             ? VK_BUFFER_USAGE_2_UNIFORM_BUFFER_BIT
                                                             : VK_BUFFER_USAGE_2_STORAGE_BUFFER_BIT;
                const char* usage_vuid = (desc_type == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER)
                                             ? "VUID-VkResourceDescriptorInfoEXT-type-11461"
                                             : "VUID-VkResourceDescriptorInfoEXT-type-11462";
                skip |= ValidateDeviceAddressRange(address_range.address, address_range.size, false,
                                                   resource_data_loc.dot(Field::pAddressRange), LogObjectList(device), buffer_usage,
                                                   usage_vuid);
            } else if (desc_type == VK_DESCRIPTOR_TYPE_PARTITIONED_ACCELERATION_STRUCTURE_NV) {
                BufferAddressValidation<1> buffer_address_validator = {
                    {{{"VUID-VkResourceDescriptorInfoEXT-type-11466",
                       [](const vvl::Buffer& buffer_state) {
                           return (buffer_state.usage & VK_BUFFER_USAGE_2_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR) == 0;
                       },
                       []() {
                ...
                skip |= buffer_address_validator.ValidateDeviceAddress(
                    *this, resource_data_loc.dot(Field::pAddressRange).dot(Field::address), LogObjectList(device),
                    address_range.address);
            } else if (desc_type == VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_NV) {
                if (address_range.size != 0) {
                    skip |= LogError("VUID-VkResourceDescriptorInfoEXT-type-11468", device,
                                     resource_data_loc.dot(Field::pAddressRange).dot(Field::size), "is %" PRIu64 ", but must be 0.",
                                     address_range.size);
                }
            } else {
    ...
    for (uint32_t i = 0; i < imageCount; ++i) {
        if (pDatas[i].size != phys_dev_ext_props.descriptor_heap_props.imageCaptureReplayOpaqueDataSize) {
            skip |= LogError("VUID-vkGetImageOpaqueCaptureDataEXT-size-11283", device,
                             ...
                             "is %" PRIu64 ", that is not equal to imageCaptureReplayOpaqueDataSize (%" PRIu64 ").", pDatas[i].size,
...
std::string CoreChecks::OverlapDataToString(const std::vector<vvl::DeviceState::CommandBufferOverlapData>& overlap_data,
                                            bool sampler) const {
    std::stringstream css;

    for (size_t i = 0; i < overlap_data.size(); i++) {
        css << " * " << (sampler ? "sampler" : "resource") << " descriptor heap reserved area "
            << string_range_hex(overlap_data[i].second) << " bound to " << FormatHandle(overlap_data[i].first->Handle()) << "\n";
    }
...
bool CoreChecks::PreCallValidateCmdPushDataEXT(VkCommandBuffer commandBuffer, const VkPushDataInfoEXT* pPushDataInfo,
    ...
    if (pPushDataInfo->offset + pPushDataInfo->data.size > phys_dev_ext_props.descriptor_heap_props.maxPushDataSize) {
        skip |= LogError("VUID-VkPushDataInfoEXT-offset-11243", commandBuffer,
                         ...
                         "(%" PRIu32 ") + pPushDataInfo->data.size (%" PRIu64 ") is greater than maxPushDataSize (%" PRIu64 ").",
    ...
    if (!IsIntegerMultipleOf(pPushDataInfo->data.size, 4)) {
        skip |= LogError("VUID-VkPushDataInfoEXT-data-11419", commandBuffer,
                         ...
                         "(%" PRIu64 ") is not a multiple of 4.", pPushDataInfo->data.size);
    }
...
bool CoreChecks::ValidateInheritanceDescriptorHeapInfo(const vvl::CommandBuffer& cb_state, const Location& loc) const {
    ...
    if (cb_state.inheritance_descriptor_heap_info.pSamplerHeapBindInfo) {
        ...
        skip |= LogError(vuid, objlist, loc.dot(Field::commandBuffer),
                         "began with VkCommandBufferInheritanceDescriptorHeapInfoEXT::pSamplerHeapBindInfo = %p",
                         cb_state.inheritance_descriptor_heap_info.pSamplerHeapBindInfo);
    }
    ...
    if (cb_state.inheritance_descriptor_heap_info.pResourceHeapBindInfo) {
        ...
        const LogObjectList objlist(cb_state.Handle(), cb_state.allocate_info.commandPool);
        skip |= LogError(vuid, objlist, loc.dot(Field::commandBuffer),
                         "began with VkCommandBufferInheritanceDescriptorHeapInfoEXT::pResourceHeapBindInfo = %p",
                         cb_state.inheritance_descriptor_heap_info.pResourceHeapBindInfo);
    }
...
bool CoreChecks::ValidateEmbeddedSamplersCount(const char* vuid, const Location& loc) const {
    bool skip = false;

    const size_t samplers_count = Count<vvl::Sampler>();

    const size_t maxSamplerAllocationCount =
        phys_dev_props.limits.maxSamplerAllocationCount -
        SafeDivision(phys_dev_ext_props.descriptor_heap_props.minSamplerHeapReservedRangeWithEmbedded,
                     phys_dev_ext_props.descriptor_heap_props.samplerDescriptorSize);

    if (samplers_count >= maxSamplerAllocationCount) {
        skip |= LogError(vuid, device, loc,
                         "contains embedded samplers, but on this device there are currently %zu VkSampler that were allocated "
                         "which is at the limit of %zu\nmaxSamplerAllocationCount = %" PRIu32
                         "\nminSamplerHeapReservedRangeWithEmbedded = %" PRIu64 "\nsamplerDescriptorSize = %" PRIu64 "\n",
                         samplers_count, maxSamplerAllocationCount, phys_dev_props.limits.maxSamplerAllocationCount,
...
bool CoreChecks::ValidateEmbeddedSamplersCount(const char* vuid, size_t new_sampler_count, const Location& loc) const {
    bool skip = false;

    const size_t samplers_count = Count<vvl::Sampler>();

    if (samplers_count + new_sampler_count >= phys_dev_ext_props.descriptor_heap_props.maxDescriptorHeapEmbeddedSamplers) {
        skip |=
            LogError(vuid, device, loc,
                     "contains %" PRIu64 " embedded samplers, but on this device there are currently %" PRIu64
