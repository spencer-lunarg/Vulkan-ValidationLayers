...
bool CoreChecks::PreCallValidateUpdateIndirectExecutionSetPipelineEXT(
    ...
    for (uint32_t i = 0; i < executionSetWriteCount; i++) {
        ...
        if (const auto initial_pipeline = indirect_execution_set->initial_pipeline) {
            ...
            const VkPipelineCreateFlags2KHR initial_pipeline_descriptor_heap =
                (initial_pipeline->create_flags & VK_PIPELINE_CREATE_2_DESCRIPTOR_HEAP_BIT_EXT) != 0;
            if (initial_pipeline_layout && update_pipeline_layout && !initial_pipeline_descriptor_heap) {
            ...
            const VkPipelineCreateFlags2KHR update_pipeline_descriptor_heap =
                (update_pipeline->create_flags & VK_PIPELINE_CREATE_2_DESCRIPTOR_HEAP_BIT_EXT);
            if (initial_pipeline_descriptor_heap != update_pipeline_descriptor_heap) {
                const LogObjectList objlist(initial_pipeline->Handle(), update_pipeline->Handle());
                const char* vuid = initial_pipeline_descriptor_heap
                ...
                skip |= LogError(
                    ...
                    update_pipeline_descriptor_heap ? "" : "not ", initial_pipeline_descriptor_heap ? "" : "not ");
            }
        }
    }
...
bool CoreChecks::PreCallValidateUpdateIndirectExecutionSetShaderEXT(VkDevice device, VkIndirectExecutionSetEXT indirectExecutionSet,
    ...
    const bool has_init_flag =
        init_shader_object ? init_shader_object->create_info.flags & VK_SHADER_CREATE_DESCRIPTOR_HEAP_BIT_EXT : false;
    ...
    for (uint32_t i = 0; i < executionSetWriteCount; i++) {
        ...
        if (indirect_execution_set->safe_create_info.info.pShaderInfo->shaderCount > 0) {
            if (init_shader_object && update_shader_object) {
                const bool has_update_flag = update_shader_object->create_info.flags & VK_SHADER_CREATE_DESCRIPTOR_HEAP_BIT_EXT;
                if (has_init_flag != has_update_flag) {
                    const LogObjectList objlist(init_shader_object->Handle(), update_shader_object->Handle());
                    const char* vuid = has_init_flag ? "VUID-vkUpdateIndirectExecutionSetShaderEXT-pInitialShaders-11327"
                    ...
                    skip |= LogError(vuid, objlist, set_write_loc.dot(Field::shader),
                                     ...
                                     has_update_flag ? "" : "not ", has_init_flag ? "" : "not ");
                }
            }
        }
    }
