...
bool CoreChecks::ValidateCreateShadersSpirv(uint32_t createInfoCount, const VkShaderCreateInfoEXT* pCreateInfos,
    ...
    for (uint32_t i = 0; i < createInfoCount; ++i) {
        ...
        const size_t embedded_samplers_count = CountDescriptorHeapEmbeddedSamplers(create_info.pNext);
        if ((create_info.flags & VK_SHADER_CREATE_DESCRIPTOR_HEAP_BIT_EXT) != 0 && embedded_samplers_count > 0) {
            skip |= ValidateEmbeddedSamplersCount("VUID-vkCreateShadersEXT-pCreateInfos-11413", loc.dot(Field::pCreateInfos, i));
            skip |= ValidateEmbeddedSamplersCount("VUID-vkCreateShadersEXT-pCreateInfos-11428", embedded_samplers_count,
                                                  loc.dot(Field::pCreateInfos, i));
        }
