...
void Validator::PreCallRecordCreateDevice(VkPhysicalDevice physicalDevice, const VkDeviceCreateInfo *pCreateInfo,
    ...
    if (gpuav_settings.shader_instrumentation.ray_query) {
        ...
        VkPhysicalDeviceFeatures2 features_2 = vku::InitStructHelper(&ray_query_feature);
        DispatchGetPhysicalDeviceFeatures2(physicalDevice, &features_2);
        if (ray_query_feature.rayQuery) {
            auto add_ray_query_feature = [this, &record_obj, modified_create_info]() {
                if (auto *ray_query_feature = const_cast<VkPhysicalDeviceRayQueryFeaturesKHR *>(
                    ...
                    InternalWarning(device, record_obj.location,
                                    "Adding a VkPhysicalDeviceRayQueryFeaturesKHR to pNext with rayQuery "
                                    "set to VK_TRUE");
            ...
            if (IsExtensionAvailable(VK_KHR_RAY_QUERY_EXTENSION_NAME, available_extensions)) {
                // Add our new extensions, only if not found
                vku::AddExtension(*modified_create_info, VK_KHR_RAY_QUERY_EXTENSION_NAME);
                add_ray_query_feature();
            }
        }
    }
}
