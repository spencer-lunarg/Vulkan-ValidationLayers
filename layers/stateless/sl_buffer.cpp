...
bool Device::manual_PreCallValidateCreateBuffer(VkDevice device, const VkBufferCreateInfo *pCreateInfo,
    ...
    if (pCreateInfo->flags & VK_BUFFER_CREATE_PROTECTED_BIT) {
        ...
        if (!phys_dev_ext_props.descriptor_heap_props.protectedDescriptorHeaps) {
            if ((usage & VK_BUFFER_USAGE_2_DESCRIPTOR_HEAP_BIT_EXT) != 0) {
                const char* vuid = usage_flags2 ? "VUID-VkBufferCreateInfo-flags-11280" : "VUID-VkBufferCreateInfo-flags-11279";
    ...
    if ((pCreateInfo->flags & (VK_BUFFER_CREATE_SPARSE_BINDING_BIT | VK_BUFFER_CREATE_SPARSE_RESIDENCY_BIT |
        ...
        if ((usage & VK_BUFFER_USAGE_DESCRIPTOR_HEAP_BIT_EXT) != 0) {
            if (!phys_dev_ext_props.descriptor_heap_props.sparseDescriptorHeaps) {
                const char* vuid = usage_flags2 ? "VUID-VkBufferCreateInfo-flags-11278" : "VUID-VkBufferCreateInfo-flags-11277";
                skip |= LogError(vuid, device, create_info_loc.dot(Field::flags),
