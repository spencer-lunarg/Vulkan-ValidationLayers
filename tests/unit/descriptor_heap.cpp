...
size_t DescriptorHeapTest::AlignedAppend(size_t& end, VkDescriptorType type, uint32_t count) {
    size_t size = 0;
    size_t align = 0;
...
size_t DescriptorHeapTest::AlignResource(size_t offset) {
    return Align(Align(offset, heap_props.bufferDescriptorAlignment), heap_props.imageDescriptorAlignment);
}

size_t DescriptorHeapTest::AlignSampler(size_t offset) { return Align(offset, heap_props.samplerDescriptorAlignment); }
...
TEST_F(NegativeDescriptorHeap, NotEnabled) {
...

    {
        ...
        VkHostAddressRangeEXT descriptors = {&sampler_ci, heap_props.samplerDescriptorSize};
    ...
    }
    {
        ...
        VkHostAddressRangeEXT descriptors = {&desc_info, heap_props.imageDescriptorSize};
...
TEST_F(NegativeDescriptorHeap, SamplerParameterSize) {
    ...
    std::vector<uint8_t> data(sampler_size);

    VkSamplerCreateInfo sampler_ci = vku::InitStructHelper();
    VkHostAddressRangeEXT descriptors = {data.data(), sampler_size - 1};
    {
...
TEST_F(NegativeDescriptorHeap, SamplerParameterYCbCr) {
    ...
    VkHostAddressRangeEXT descriptors = {data.data(), sampler_size};
...
TEST_F(NegativeDescriptorHeap, SamplerParameterPSamplersCore) {
    ...
    m_errorMonitor->SetDesiredError("VUID-VkSamplerCreateInfo-minFilter-01645");
    m_errorMonitor->SetDesiredError("VUID-vkWriteSamplerDescriptorsEXT-pSamplers-11204");  // we cannot skip this
    vk::WriteSamplerDescriptorsEXT(device(), 1u, &sampler_ci, &descriptors);
    m_errorMonitor->VerifyFound();
}
...
TEST_F(NegativeDescriptorHeap, ResourceParameterSize) {
    ...
    for (auto t : types) {
        ...
        if (size > 0) {
            ...
            VkHostAddressRangeEXT descriptors = {data.data(), size - 1};
...
TEST_F(NegativeDescriptorHeap, ResourceParameterDataNull) {
    ...
    for (const auto& s : subtests) {
        for (auto type : s.types) {
            ...
            VkHostAddressRangeEXT descriptors = {data.data(), size};
...
TEST_F(NegativeDescriptorHeap, ResourceParameterDebugUtilsObjectName) {
    ...
    std::vector<uint8_t> data(resource_size);
    ...
    device_address_range.size = buffer_size;
    device_address_range.address = dummy_buffer.Address();
    ...
    VkHostAddressRangeEXT descriptors = {data.data(), resource_size};
...
TEST_F(NegativeDescriptorHeap, ResourceParameterDataAddressZero) {
...

    {
        ...
        VkDeviceAddressRangeEXT invalid_device_address_range;
        invalid_device_address_range.size = 16;
        invalid_device_address_range.address = 0;
        VkResourceDescriptorInfoEXT resource_desc_info = vku::InitStructHelper();
        resource_desc_info.type = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
...
TEST_F(NegativeDescriptorHeap, ResourceParameterDataSizeZero) {
    ...
    for (auto type : types) {
        ...
        VkDeviceAddressRangeEXT invalid_device_address_range;
        invalid_device_address_range.size = 0;
...
TEST_F(NegativeDescriptorHeap, UniformTexelBufferOffsetSingleTexelAlignmentFalse) {
    ...
    auto data = std::vector<uint8_t>(bdsize);
    VkHostAddressRangeEXT descriptors = {&data[0], data.size()};
...
TEST_F(NegativeDescriptorHeap, ResourceParameterSubsampledImage) {
    ...
    for (auto type : types) {
        ...
        VkHostAddressRangeEXT descriptors = {data.data(), size2 - 1};
...
TEST_F(NegativeDescriptorHeap, ResourceParameterPView) {
    ...
    std::vector<uint8_t> data(heap_props.imageDescriptorSize);
...
TEST_F(NegativeDescriptorHeap, CmdBindSamplerHeap) {
    ...
    if (heap_props.samplerDescriptorSize + 1 != 0) {
        ...
        bind_info.heapRange.address = heap.Address();
        bind_info.heapRange.size = descriptor_size + heap_props.minSamplerHeapReservedRange;
    ...
    if (!overflow && max_buffer_size <= props11.maxMemoryAllocationSize) {
        ...
        // reservedRangeOffset check
        VkBindHeapInfoEXT bind_info = vku::InitStructHelper();
        bind_info.heapRange.address = max_buffer.Address();
        bind_info.heapRange.size = max_buffer_size;
...
TEST_F(NegativeDescriptorHeap, CmdBindSamplerHeapAlign) {
...

    {
        // Address alignment check
        VkBindHeapInfoEXT bind_info = vku::InitStructHelper();
        bind_info.heapRange.address = heap.Address() + 1;
        bind_info.heapRange.size = heap_size - 1;
        bind_info.reservedRangeOffset = 0;
        bind_info.reservedRangeSize = heap_props.minSamplerHeapReservedRange;
...
TEST_F(NegativeDescriptorHeap, CmdBindResourceHeap) {
    TEST_DESCRIPTION("Validate vkCmdBindResourceHeapEXT");
    AddRequiredExtensions(VK_KHR_BUFFER_DEVICE_ADDRESS_EXTENSION_NAME);
    AddRequiredFeature(vkt::Feature::bufferDeviceAddress);
    RETURN_IF_SKIP(InitBasicDescriptorHeap());
    ...
    if (!overflow && max_buffer_size <= props11.maxMemoryAllocationSize) {
        ...
        VkBindHeapInfoEXT bind_info = vku::InitStructHelper();
        bind_info.heapRange.address = max_buffer.Address();
        bind_info.heapRange.size = heap_props.maxResourceHeapSize + heap_props.bufferDescriptorSize;
        bind_info.reservedRangeOffset = 0;
...
TEST_F(NegativeDescriptorHeap, SamplerInheritance) {
    ...
    vkt::CommandBuffer secondary(*m_device, m_command_pool, VK_COMMAND_BUFFER_LEVEL_SECONDARY);
    {
        ...
        VkBindHeapInfoEXT bind_info = vku::InitStructHelper();
        bind_info.heapRange.address = heap.Address();
        bind_info.heapRange.size = heap_size / 2;
        bind_info.reservedRangeOffset = 0;
        bind_info.reservedRangeSize = heap_props.minSamplerHeapReservedRange;
...
TEST_F(NegativeDescriptorHeap, CmdBindResourceHeapAlign) {
    TEST_DESCRIPTION("Validate vkCmdBindResourceHeapEXT addr alignement");
    AddRequiredExtensions(VK_KHR_BUFFER_DEVICE_ADDRESS_EXTENSION_NAME);
    AddRequiredFeature(vkt::Feature::bufferDeviceAddress);
    RETURN_IF_SKIP(InitBasicDescriptorHeap());
...
TEST_F(NegativeDescriptorHeap, CmdBindResourceHeapSecondaryBufferMemoryTests) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        VkBindHeapInfoEXT bind_info = vku::InitStructHelper();
        bind_info.heapRange.address = heap.Address();
        bind_info.heapRange.size = (i == 0) ? heap_size + heap_props.samplerDescriptorAlignment : heap_size;
    ...
    for (int i = 0; i < 2; i++) {
        ...
        VkBindHeapInfoEXT bind_info = vku::InitStructHelper();
        bind_info.heapRange.address = heap.Address();
        bind_info.heapRange.size = (i == 0) ? heap_size + heap_props.resourceHeapAlignment : heap_size;
        bind_info.reservedRangeOffset = 0;
        bind_info.reservedRangeSize = heap_props.minResourceHeapReservedRange;
...
TEST_F(NegativeDescriptorHeap, CmdPushData) {
    TEST_DESCRIPTION("Validate vkCmdPushDataEXT function");
    AddRequiredExtensions(VK_KHR_BUFFER_DEVICE_ADDRESS_EXTENSION_NAME);
    AddRequiredFeature(vkt::Feature::bufferDeviceAddress);
    RETURN_IF_SKIP(InitBasicDescriptorHeap());

    std::vector<uint8_t> payload(4 * heap_props.maxPushDataSize);
...
TEST_F(NegativeDescriptorHeap, ShaderDescriptorSetAndBindingMapping) {
    TEST_DESCRIPTION("Validate VkShaderDescriptorSetAndBindingMappingInfoEXT structure");
    RETURN_IF_SKIP(InitBasicDescriptorHeap());
    InitRenderTarget();

    VkShaderDescriptorSetAndBindingMappingInfoEXT mapping_info = vku::InitStructHelper();
    VkDescriptorSetAndBindingMappingEXT mapping[] = {vku::InitStructHelper(), vku::InitStructHelper()};
...

    {
        ...
        mapping[0].source = VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_CONSTANT_OFFSET_EXT;
        mapping[0].resourceMask = VK_SPIRV_RESOURCE_TYPE_SAMPLER_BIT_EXT;
        mapping[0].firstBinding = 0;
        mapping[0].bindingCount = 10;
        mapping[0].descriptorSet = 0;
        mapping[1].source = VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_CONSTANT_OFFSET_EXT;
        mapping[1].resourceMask = VK_SPIRV_RESOURCE_TYPE_SAMPLER_BIT_EXT;
        mapping[1].firstBinding = 2;
        mapping[1].bindingCount = 5;
...

    {
        ...
        mapping[0].firstBinding = 0;
        mapping[0].bindingCount = 10;
        mapping[0].descriptorSet = 1;
        mapping[1].source = VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_CONSTANT_OFFSET_EXT;
        mapping[1].resourceMask = VK_SPIRV_RESOURCE_TYPE_SAMPLER_BIT_EXT;
        mapping[1].firstBinding = 2;
        mapping[1].bindingCount = 10;
        mapping[1].descriptorSet = 1;
...

    {
        ...
        mapping[0].source = VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_CONSTANT_OFFSET_EXT;
        mapping[0].resourceMask = VK_SPIRV_RESOURCE_TYPE_SAMPLER_BIT_EXT;
        mapping[0].firstBinding = 20;
        mapping[0].bindingCount = UINT32_MAX;
        mapping[0].descriptorSet = 2;
        mapping[1].source = VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_CONSTANT_OFFSET_EXT;
        mapping[1].resourceMask = VK_SPIRV_RESOURCE_TYPE_SAMPLER_BIT_EXT;
        mapping[1].firstBinding = 30;
        mapping[1].bindingCount = 10;
        mapping[1].descriptorSet = 2;
...
TEST_F(NegativeDescriptorHeap, DescriptorSetAndBindingMappingPipeline) {
    ...
    for (auto source :
        ...
        mapping = vku::InitStructHelper();
        mapping.source = source;
        mapping.resourceMask = VK_SPIRV_RESOURCE_TYPE_READ_ONLY_IMAGE_BIT_EXT;
        mapping.bindingCount = 1;
...
TEST_F(NegativeDescriptorHeap, DescriptorMappingSourcePushIndex) {
    TEST_DESCRIPTION("Validate VkDescriptorMappingSourcePushIndexEXT structure");
    RETURN_IF_SKIP(InitBasicDescriptorHeap());
...
TEST_F(NegativeDescriptorHeap, DescriptorMappingSourceIndirectIndex) {
    TEST_DESCRIPTION("Validate VkDescriptorMappingSourceIndirectIndexEXT structure");
    RETURN_IF_SKIP(InitBasicDescriptorHeap());
...
TEST_F(NegativeDescriptorHeap, DescriptorMappingSourceHeapData) {
    TEST_DESCRIPTION("Validate VkDescriptorMappingSourceHeapDataEXT structure");
    RETURN_IF_SKIP(InitBasicDescriptorHeap());
...
TEST_F(NegativeDescriptorHeap, DescriptorMappingSourceIndirectAddress) {
    TEST_DESCRIPTION("Validate VkDescriptorMappingSourceIndirectAddressEXT structure");
    RETURN_IF_SKIP(InitBasicDescriptorHeap());
...
TEST_F(NegativeDescriptorHeap, DescriptorMappingSourceShaderRecordIndexAlign) {
    TEST_DESCRIPTION("Validate VkDescriptorMappingSourceShaderRecordIndexEXT structure");
    RETURN_IF_SKIP(InitBasicDescriptorHeap());
...
TEST_F(NegativeDescriptorHeap, DescriptorMappingSourceIndirectIndexArray) {
    TEST_DESCRIPTION("Validate VkDescriptorMappingSourceIndirectIndexArrayEXT structure");
    RETURN_IF_SKIP(InitBasicDescriptorHeap());
...
TEST_F(NegativeDescriptorHeap, OpaqueCaptureDataCreateInfo) {
    TEST_DESCRIPTION("Validate VkOpaqueCaptureDataCreateInfoEXT structure");
    RETURN_IF_SKIP(InitBasicDescriptorHeap());
...
TEST_F(NegativeDescriptorHeap, GetImageOpaqueCaptureData) {
    TEST_DESCRIPTION("Validate vkGetImageOpaqueCaptureDataEXT function");
    SetTargetApiVersion(VK_API_VERSION_1_2);
    AddRequiredFeature(vkt::Feature::descriptorHeapCaptureReplay);
    AddRequiredFeature(vkt::Feature::bufferDeviceAddress);
...
TEST_F(NegativeDescriptorHeap, RegisterCustomBorderColor) {
    TEST_DESCRIPTION("Validate vkRegisterCustomBorderColorEXT function");
    SetTargetApiVersion(VK_API_VERSION_1_2);
    AddRequiredExtensions(VK_EXT_CUSTOM_BORDER_COLOR_EXTENSION_NAME);
    AddRequiredFeature(vkt::Feature::customBorderColors);
    RETURN_IF_SKIP(InitBasicDescriptorHeap());
...
TEST_F(NegativeDescriptorHeap, UnregisterCustomBorderColor) {
    TEST_DESCRIPTION("Validate vkUnregisterCustomBorderColorEXT function");
    SetTargetApiVersion(VK_API_VERSION_1_2);
    AddRequiredExtensions(VK_EXT_CUSTOM_BORDER_COLOR_EXTENSION_NAME);
    AddRequiredFeature(vkt::Feature::customBorderColors);
    RETURN_IF_SKIP(InitBasicDescriptorHeap());
...
TEST_F(NegativeDescriptorHeap, SamplerCreateWithBorder) {
    TEST_DESCRIPTION("Validate VkSamplerCustomBorderColorIndexCreateInfoEXT in VkSamplerCreateInfo");
    SetTargetApiVersion(VK_API_VERSION_1_2);
    AddRequiredExtensions(VK_EXT_CUSTOM_BORDER_COLOR_EXTENSION_NAME);
    AddRequiredFeature(vkt::Feature::customBorderColors);
    RETURN_IF_SKIP(InitBasicDescriptorHeap());
...
TEST_F(NegativeDescriptorHeap, DescriptorHeapPipelineCreateFlag) {
    TEST_DESCRIPTION("Validate pipeline descriptor heap flag in pipeline creation.");
    RETURN_IF_SKIP(InitBasicDescriptorHeap());
...
TEST_F(NegativeDescriptorHeap, ShaderCreateInfoPushConstant) {
    TEST_DESCRIPTION("Validate ShaderCreateInfo with VkPushConstantRange handled properly");
    AddRequiredExtensions(VK_EXT_SHADER_OBJECT_EXTENSION_NAME);
    AddRequiredFeature(vkt::Feature::shaderObject);
    RETURN_IF_SKIP(InitBasicDescriptorHeap());
...
TEST_F(NegativeDescriptorHeap, DescriptorType) {
    TEST_DESCRIPTION("Validate vkGetPhysicalDeviceDescriptorSizeEXT DescriptorType parameter");
    RETURN_IF_SKIP(InitBasicDescriptorHeap());
...
TEST_F(NegativeDescriptorHeap, BindHeapInfoBufferHeapUsage) {
...

    {
        ...
        VkBindHeapInfoEXT bind_info = vku::InitStructHelper();
        bind_info.heapRange.address = buffer.Address();
        bind_info.heapRange.size = bdsize;
        bind_info.reservedRangeSize = heap_props.minSamplerHeapReservedRange;
    ...
    for (int i = 0; i < 2; i++) {
        ...
        VkBindHeapInfoEXT bind_info = vku::InitStructHelper();
        bind_info.heapRange.address = buffer.Address();
        bind_info.heapRange.size = (i == 0) ? bdsize + 1 : bdsize;
        bind_info.reservedRangeSize = heap_props.minSamplerHeapReservedRange;
    ...
    for (int i = 0; i < 2; i++) {
        ...
        VkBindHeapInfoEXT bind_info = vku::InitStructHelper();
        bind_info.heapRange.address = heap.Address();
        bind_info.heapRange.size = (i == 0) ? heap_size + 1 : heap_size;
        bind_info.reservedRangeSize = heap_props.minResourceHeapReservedRange;
...
TEST_F(NegativeDescriptorHeap, BindOverlappingRangesSampler) {
    TEST_DESCRIPTION("Validate reservedRangeOffset improperly bind to sampler heap");
    AddRequiredExtensions(VK_KHR_BUFFER_DEVICE_ADDRESS_EXTENSION_NAME);
    AddRequiredFeature(vkt::Feature::bufferDeviceAddress);
    RETURN_IF_SKIP(InitBasicDescriptorHeap());
...
TEST_F(NegativeDescriptorHeap, WriteResourceDescriptorsMemoryTests) {
    TEST_DESCRIPTION("Validate vkWriteResourceDescriptorsEXT buffer memory bound is proper");
    AddRequiredExtensions(VK_KHR_BUFFER_DEVICE_ADDRESS_EXTENSION_NAME);
    AddRequiredFeature(vkt::Feature::bufferDeviceAddress);
    RETURN_IF_SKIP(InitBasicDescriptorHeap());
    ...
    for (int i = 0; i < 2; i++) {
        ...
        auto data = std::vector<uint8_t>(buffer_size);
        VkHostAddressRangeEXT descriptors = {&data[0], data.size()};

        texel_buffer_info.addressRange.address = buffer.Address();
        texel_buffer_info.addressRange.size = (i == 0) ? 2 * buffer_size : buffer_size;
...
TEST_F(NegativeDescriptorHeap, WriteResourceDescriptorsMemoryTestsAS) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        address_range.address = as_buffer.Address();
        address_range.size = (i == 0) ? as_size + 256 : as_size;
...
TEST_F(NegativeDescriptorHeap, OpTypeImage) {
    ...
    VkDescriptorSetAndBindingMappingEXT mappings[1] = {vku::InitStructHelper()};
    ...
    mappings[0].sourceData.constantOffset.heapOffset = 1;
    ...
    for (int i = 0; i < 4; i++) {
        ...
        pipe.cp_ci_.stage.flags = 0;
        pipe.cp_ci_.stage.stage = VK_SHADER_STAGE_COMPUTE_BIT;
        pipe.cp_ci_.stage.module = cs_modules[i / 2];
        pipe.cp_ci_.stage.pName = "main";
        pipe.cp_ci_.stage.pNext = &mapping_info;
        pipe.cp_ci_.stage.pSpecializationInfo = nullptr;

        mappings[0].sourceData.constantOffset.heapOffset = (i % 2 == 0) ? 1 : 0;
        mappings[0].sourceData.constantOffset.heapArrayStride = (i % 2 == 0) ? 0 : 1;
...
TEST_F(NegativeDescriptorHeap, MappedPushIsBlockUniform) {
    ...
    CreateComputePipelineHelper pipe(*this, &pipeline_create_flags_2_create_info);
    pipe.cp_ci_.layout = VK_NULL_HANDLE;
    pipe.cp_ci_.stage = vku::InitStructHelper(&mapping_info);
    pipe.cp_ci_.stage.stage = VK_SHADER_STAGE_COMPUTE_BIT;
    pipe.cp_ci_.stage.pName = "main";

    for (int i = 0; i < 2; ++i) {
        pipe.cp_ci_.stage.module = cs_modules[i];
...
TEST_F(NegativeDescriptorHeap, MappedStructLessThanMaxPushDataSize) {
    ...
    VkDescriptorSetAndBindingMappingEXT mappings[2] = {vku::InitStructHelper(), vku::InitStructHelper()};
    mappings[0].descriptorSet = 0;
    mappings[0].firstBinding = 0;
    mappings[0].bindingCount = 1;
    mappings[0].resourceMask = VK_SPIRV_RESOURCE_TYPE_ALL_EXT;
...
TEST_F(NegativeDescriptorHeap, OpTypeStruct) {
    TEST_DESCRIPTION("Validate that mapping is aligned for OpTypeStruct");
    AddRequiredFeature(vkt::Feature::bufferDeviceAddress);
    RETURN_IF_SKIP(InitBasicDescriptorHeap());
    RETURN_IF_SKIP(CheckSlangSupport());
...
TEST_F(NegativeDescriptorHeap, OpTypeSampler) {
    TEST_DESCRIPTION("Validate that mapping is aligned for OpTypeSampler");
    ...
    const VkDeviceSize sampler_heap_size_app = AlignSampler(sampler_descriptor_count * sampler_stride);
    ...
    VkDescriptorSetAndBindingMappingEXT mappings[3] = {vku::InitStructHelper(), vku::InitStructHelper(), vku::InitStructHelper()};
    mappings[0].descriptorSet = 0;
    mappings[0].firstBinding = 0;
    mappings[0].bindingCount = 1;
    mappings[0].resourceMask = VK_SPIRV_RESOURCE_TYPE_ALL_EXT;
    ...
    for (int j = 0; j < 4; j++) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            pipe.cp_ci_.stage.flags = 0;
            pipe.cp_ci_.stage.stage = VK_SHADER_STAGE_COMPUTE_BIT;
            pipe.cp_ci_.stage.module = cs_modules[j];
            pipe.cp_ci_.stage.pName = "main";
            pipe.cp_ci_.stage.pNext = &mapping_info;
            pipe.cp_ci_.stage.pSpecializationInfo = nullptr;
...
TEST_F(NegativeDescriptorHeap, NoMappingComputePipeline) {
    ...
    const VkDeviceSize resource_heap_size_app = AlignResource(resource_descriptor_count * resource_stride);
    const VkDeviceSize resource_heap_size = resource_heap_size_app + heap_props.minResourceHeapReservedRange;
    const VkDeviceSize dataBufferSize = 4u * sizeof(uint32_t);
...
TEST_F(NegativeDescriptorHeap, PushDataAssignedShaderObject) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        VkShaderDescriptorSetAndBindingMappingInfoEXT mapping_info = vku::InitStructHelper();
        mapping_info.mappingCount = sizeof(mappings) / sizeof(mappings[0]);
        mapping_info.pMappings = mappings;
...
TEST_F(NegativeDescriptorHeap, PushDataRange) {
    ...
    device_range.size = sizeof(float) * 4u;
...
TEST_F(NegativeDescriptorHeap, SamplerAllocationCountPipeline) {
    ...
    const auto source = VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_CONSTANT_OFFSET_EXT;
    mapping = vku::InitStructHelper();
    mapping.resourceMask = VK_SPIRV_RESOURCE_TYPE_READ_ONLY_IMAGE_BIT_EXT;
    mapping.bindingCount = 1;
    mapping.source = source;
...
TEST_F(NegativeDescriptorHeap, SamplerAllocationTotalCountPipeline) {
    ...
    const int max_samplers = heap_props.maxDescriptorHeapEmbeddedSamplers - 1;
    ...
    const auto source = VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_CONSTANT_OFFSET_EXT;
    mapping[0].resourceMask = VK_SPIRV_RESOURCE_TYPE_READ_ONLY_IMAGE_BIT_EXT;
    mapping[0].bindingCount = 1;
    mapping[0].source = source;
...
TEST_F(NegativeDescriptorHeap, UsagesValidation) {
    ...
    }
    {
        ...
        VkHostAddressRangeEXT descriptors = {&desc_info, heap_props.bufferDescriptorSize};
...
TEST_F(NegativeDescriptorHeap, InvalidateComputeBoundDescriptorSetsBindDescriptorSets) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (i == 0) {
            ...
            std::vector<uint8_t> payload(heap_props.maxPushDataSize / 2);
            VkPushDataInfoEXT push_data_info = vku::InitStructHelper();
            push_data_info.data.address = payload.data();
            push_data_info.data.size = payload.size();
            push_data_info.offset = 0;
...
TEST_F(NegativeDescriptorHeap, SecondaryCmdBufferSamplerHeapUnbound) {
    ...
    VkHostAddressRangeEXT sampler_host;
    sampler_host.address = sampler_heap_data + sampler_offset;
    sampler_host.size = sampler_size;
    vk::WriteSamplerDescriptorsEXT(*m_device, 1u, &sampler_info, &sampler_host);

    VkBindHeapInfoEXT sampler_bind_info = vku::InitStructHelper();
    sampler_bind_info.heapRange.address = sampler_heap.Address();
    ...
    resource_bind_info.heapRange.address = resource_heap_.Address();
    resource_bind_info.heapRange.size = resource_heap_.CreateInfo().size;
