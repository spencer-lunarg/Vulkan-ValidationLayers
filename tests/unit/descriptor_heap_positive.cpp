...
TEST_F(PositiveDescriptorHeap, SamplerInheritance) {
    ...
    bind_info.heapRange.address = sampler_heap_.Address();
    bind_info.heapRange.size = sampler_heap_.CreateInfo().size;
...
TEST_F(PositiveDescriptorHeap, ResourceInheritance) {
    ...
    VkBindHeapInfoEXT bind_info = vku::InitStructHelper();
    bind_info.heapRange.address = resource_heap_.Address();
    bind_info.heapRange.size = resource_heap_.CreateInfo().size;
    bind_info.reservedRangeOffset = 0;
...
TEST_F(PositiveDescriptorHeap, CombinedImageSampler) {
    ...
    sampler_host.size = heap_props.samplerDescriptorSize;
    vk::WriteSamplerDescriptorsEXT(*m_device, 1u, &sampler_info, &sampler_host);
    ...
    mappings[0].firstBinding = 0u;
    mappings[0].bindingCount = 1u;
    mappings[0].resourceMask = VK_SPIRV_RESOURCE_TYPE_COMBINED_SAMPLED_IMAGE_BIT_EXT;
    mappings[0].source = VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_CONSTANT_OFFSET_EXT;
    mappings[0].sourceData.constantOffset = {};
    mappings[1] = vku::InitStructHelper();
...
TEST_F(PositiveDescriptorHeap, ConstantMemoryAccess) {
    ...
    pipe.cp_ci_.stage.module = cs_module1;
...
TEST_F(PositiveDescriptorHeap, ResourceMask) {
    ...
    for (uint32_t i = 0; i < 8; ++i) {
        mappings[i] = vku::InitStructHelper();
        mappings[i].descriptorSet = 0;
        mappings[i].firstBinding = i;
        mappings[i].bindingCount = 1;
...
TEST_F(PositiveDescriptorHeap, OffsetIdEXT) {
    ...
    uint32_t specialization_data[3] = {0u, 32u, 96u};
    ...
    CreateComputePipelineHelper pipe(*this, &pipeline_create_flags_2_create_info);
    pipe.cp_ci_.layout = VK_NULL_HANDLE;
    pipe.cp_ci_.stage = vku::InitStructHelper(&mappingInfo);
    pipe.cp_ci_.stage.stage = VK_SHADER_STAGE_COMPUTE_BIT;
    pipe.cp_ci_.stage.module = cs_module;
    pipe.cp_ci_.stage.pName = "main";
...
TEST_F(PositiveDescriptorHeap, UboAndSsboBindings) {
    ...
    for (uint32_t i = 0; i < 16; ++i) {
        ...
        mappings[i].firstBinding = i;
        mappings[i].bindingCount = 1;
        mappings[i].resourceMask = VK_SPIRV_RESOURCE_TYPE_UNIFORM_BUFFER_BIT_EXT;
        if (i % 2 == 0) {
        ...
        mappings[16 + i] = vku::InitStructHelper();
        mappings[16 + i].descriptorSet = 0;
        mappings[16 + i].firstBinding = i;
        mappings[16 + i].bindingCount = 1;
        mappings[16 + i].resourceMask =
